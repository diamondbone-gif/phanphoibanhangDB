    ...
    <div class="card"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        ...
        <tbody>@forelse($transactions as $transaction)<tr>
            ...
            <td>{{ $transaction->created_at?->format('d/m/Y H:i') }}</td><td class="d-flex gap-1 flex-wrap">
                ...
                @if($transaction->status === \App\Enums\FinancialTransactionState::Approved)<form method="POST" enctype="multipart/form-data" action="{{ route('admin.financial-transactions.complete', $transaction) }}">@csrf @method('PATCH')<input class="form-control form-control-sm mb-1" name="bank_reference" placeholder="Mã ngân hàng"><input type="file" class="form-control form-control-sm mb-1" name="attachment" accept=".pdf,.jpg,.jpeg,.png"><button class="btn btn-sm btn-primary">Đã chi/thu</button></form>@endif
                @if($transaction->attachment_path)<a class="btn btn-sm btn-outline-secondary" href="{{ route('admin.financial-transactions.attachment', $transaction) }}">Chứng từ</a>@endif
