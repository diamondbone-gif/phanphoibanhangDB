...
class CustomerController extends Controller
{
    ...
    public function checkReferrer(Request $request)
    {
        ...
        if ($phone === '') {
            return response()->json([
                'found' => false,
                'success' => false,
                'message' => 'Vui lòng nhập số điện thoại CTV/người giới thiệu.',
            ]);
        }
        ...
        if (!$customer) {
            return response()->json([
                'found' => false,
                'success' => false,
                'message' => 'Không tìm thấy CTV/người giới thiệu với số điện thoại này.',
            ]);
        }
        ...
        return response()->json([
            ...
            'message' => 'Tìm thấy: ' . $customer->full_name . ' - ' . $customer->phone,
            'data' => [
                'id' => $customer->id,
    ...
    public function show(Customer $customer)
    {
        ...
        if (
            ...
            Schema::hasColumn('customer_needs', 'id')
        ) {
            $customerNeeds = DB::table('customer_need_maps')
                ...
                ->when(Schema::hasColumn('customer_needs', 'sort_order'), function ($query) {
                    $query->orderBy('customer_needs.sort_order');
                })
    ...
    public function edit(Customer $customer)
    {
        ...
        $currentReferrer = $this->currentReferrer($customer);

        $customerKind = $currentReferrer ? 'ctv' : 'self';

        $currentReferrerPhone = $currentReferrer?->phone ?? '';

        $currentCommissionRate = 5;

        if (
            Schema::hasTable('customer_referrals') &&
            Schema::hasColumn('customer_referrals', 'commission_rate') &&
            Schema::hasColumn('customer_referrals', 'referred_customer_id')
        ) {
            $currentCommissionRate = DB::table('customer_referrals')
                ->where('referred_customer_id', $customer->id)
                ->value('commission_rate') ?? 5;
        }
    ...
    public function update(Request $request, Customer $customer)
    {
        $request->validate([
            ...
            'customer_kind' => ['required', 'in:self,ctv'],
            'referrer_phone' => ['nullable', 'required_if:customer_kind,ctv', 'string', 'max:30'],
            ...
            'medical_note' => ['nullable', 'string', 'max:3000'],
            'consultation_note' => ['nullable', 'string', 'max:3000'],
        ...
        DB::transaction(function () use ($request, $customer) {
            ...
            $customerData = $this->filterExistingColumns('customers', $customerData);
            ...
            if (Schema::hasTable('customer_details')) {
                $detailData = [
                    ...
                    'source_channel_id' => $request->customer_kind === 'self'
                ...
                $detailData = $this->filterExistingColumns('customer_details', $detailData);

                $existingDetail = DB::table('customer_details')
                    ->where('customer_id', $customer->id)
                    ->first();

                if ($existingDetail) {
                    if (Schema::hasColumn('customer_details', 'updated_at')) {
                    ...
                    if (Schema::hasColumn('customer_details', 'created_at')) {
                    ...
                    DB::table('customer_details')->insert($detailData);
                }
            }

            $this->syncCustomerReferral($customer, $request);
            ...
            if (
                ...
                if ($request->filled('customer_need_id')) {
                    ...
                    if (Schema::hasColumn('customer_need_maps', 'created_at')) {
                        $needMapData['created_at'] = now();
                    }

                    if (Schema::hasColumn('customer_need_maps', 'updated_at')) {
                        $needMapData['updated_at'] = now();
                    }

                    DB::table('customer_need_maps')->insert($needMapData);
                }
            }
        });
    ...
    private function syncCustomerReferral(Customer $customer, Request|array $input): void
    {
        ...
        $referrerPhone = $isRequest
            ? $input->input('referrer_phone')
            : ($input['referrer_phone'] ?? null);

        $commissionRate = $isRequest
            ? ($input->input('commission_rate') ?: 5)
            : ($input['commission_rate'] ?? 5);
        ...
        if (!$referrer) {
            throw ValidationException::withMessages([
                'referrer_phone' => 'Không tìm thấy CTV/người giới thiệu.',
            ]);
        }
        ...
        if (Schema::hasTable('customer_referrals')) {
            ...
            $referralData = $this->filterExistingColumns('customer_referrals', $referralData);

            if (Schema::hasColumn('customer_referrals', 'updated_at')) {
                $referralData['updated_at'] = now();
            }

            $existing = DB::table('customer_referrals')
                ->where('referred_customer_id', $customer->id)
                ->first();

            if ($existing) {
                unset($referralData['started_at']);
                ...
                DB::table('customer_referrals')->insert($referralData);
            }
        }
    }

    private function currentReferrer(Customer $customer): ?Customer
    {
        if (
            Schema::hasTable('customer_referrals') &&
            Schema::hasColumn('customer_referrals', 'referred_customer_id') &&
            Schema::hasColumn('customer_referrals', 'referrer_customer_id')
        ) {
            ...
            if ($referrerId) {
                return Customer::query()->find($referrerId);
            }
        }

        if (
            Schema::hasColumn('customers', 'referrer_id') &&
            !empty($customer->referrer_id)
        ) {
            return Customer::query()->find($customer->referrer_id);
        }

        return null;
    }

    private function optionRows(array $tables)
    {
        foreach ($tables as $table) {
            ...
            return DB::table($table)
                ...
                ->when(Schema::hasColumn($table, 'is_active'), function ($query) {
                    $query->where('is_active', 1);
                })
                ->orderBy($nameColumn)
                ->get();
